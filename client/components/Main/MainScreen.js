...
export const MainScreen = ({ navigation }) => {
  ...
  return (
    ...
    >
      <View style={styles.headerWrapper}>
        ...
        <TouchableOpacity onPress={logoutHandler}>
          <Icon
            ...
            size={30}
            color="black"
            style={{ alignSelf: "flex-start", marginRight: 18 }}
          />
        </TouchableOpacity>
      </View>
        ...
        <FlatList
          ...
          ListFooterComponent={
            <View>
              <View style={styles.footer}>
                <TouchableOpacity onPress={createListHandler}>
                  <Text
                ...
                <TextInput
                  ...
                  value={listName}
                  onChangeText={onListNameHandler}
                />
              </View>
              <View style={styles.footer}>
                <TouchableOpacity onPress={onJoinListHandler}>
                  <Text
                    style={{ fontSize: 30, fontWeight: "bold", marginLeft: 30 }}
                  >
                    +
                ...
                <TextInput
                  style={styles.input}
                  placeholder="Join List by ID"
                  value={listId}
                  onChangeText={onListIdHandler}
                />
              </View>
            </View>
          }
          ...
          renderItem={({ item, index }) => (
            ...
            >
              ...
              >
                <Text style={styles.listName}>{item.split("-")[0]}</Text>
              </TouchableOpacity>
              <TouchableOpacity
                onPress={() => deleteListHandler(item.split("-")[1])}
              >
                <AntIcon name="logout" size={23} />
              </TouchableOpacity>
            </View>
          )}
        ></FlatList>
      ...
      >
        <Text style={styles.currentListHeader}>{`Currect List: ${
          currentList.split("-")[0]
        }`}</Text>
        <Text>{`Id: ${
          currentList.split("-")[1] ? currentList.split("-")[1] : ""
        }`}</Text>
      </View>
      <View style={styles.listWrapper}>
        <FlatList
          ...
          ListFooterComponent={
            items.length === 0 && (
              <Text
                style={{ padding: 15, letterSpacing: 1, fontWeight: "200" }}
              >
                No items found.
              </Text>
            )
      ...
      {error !== "" && (
        <Text
          style={{ color: "red", width: "100%", paddingLeft: 20, fontSize: 18 }}
        >{`${error}`}</Text>
      )}
      {!showAddItem && currentList.length !== 0 && (
        <TouchableOpacity
          onPress={() => {
            setShowLists(false);
            setShowAddItem(true);
